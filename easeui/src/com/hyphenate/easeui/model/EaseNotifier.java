...
public class EaseNotifier {
    ...
    protected void sendNotification(EMMessage message, boolean isForeground, boolean numIncrease) {
        ...
        try {
            ...
            String  title = contentTitle;
            String body = summaryBody;
            try {
                String[] split = notifyText.split(":");
                title = split[0];
                body = split[1];
            }catch (Exception e){
                e.printStackTrace();
            ...
            if (isForeground) {
                notificationManager.notify(message.getFrom(),foregroundNotifyID, notification);
                notificationManager.cancel(message.getFrom(),foregroundNotifyID);
            } else {
                notificationManager.notify(message.getFrom(),notifyID, notification);
            }
